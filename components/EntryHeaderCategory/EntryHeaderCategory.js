...
export default function EntryHeaderCategory({ databaseId }) {
  ...
  // Get Category
  const { data, loading } = useQuery(GetEntryHeaderCategory, {
  ...
  if (loading) {
    return (
      <>
        <div className="mx-auto my-0 p-4 flex max-w-[100vw] justify-center lg:max-w-[1024px]	">
          <div role="status">
            <svg
              aria-hidden="true"
            ...
            data?.category?.children != undefined && (
              <div className={cx("navbar-wrapper")}>
                ...
                <div className={cx("parent-nav-wrapper")}>
                  <Link href={data?.category?.uri}>{data?.category?.name}</Link>
                </div>
